...
def solve_qr(A, b):
    ...
    x = spsolve_triangular(csc_matrix(R).asformat('csr'), z, lower=False)
    return x.squeeze(), R
...
def solve_qr_colamd(A, b):
    ...
    x = permutation_vector_to_matrix(E) @ spsolve_triangular(csc_matrix(R).asformat('csr'), z, lower=False)
    return x.squeeze(), R
